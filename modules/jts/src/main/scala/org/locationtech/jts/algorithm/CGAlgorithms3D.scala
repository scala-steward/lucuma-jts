...
object CGAlgorithms3D {
  ...
  def distancePointSegment(p: Coordinate, A: Coordinate, B: Coordinate): Double = { // if start = end, then just compute distance to one of the endpoints
    ...
    /*
           ...
           */ val len2 = (B.x - A.x) * (B.x - A.x) + (B.y - A.y) * (B.y - A.y) + (B.getZ - A.getZ) * (B.getZ - A.getZ)
    if (java.lang.Double.isNaN(len2)) throw new IllegalArgumentException("Ordinates must not be NaN")
    val r = ((p.x - A.x) * (B.x - A.x) + (p.y - A.y) * (B.y - A.y) + (p.getZ - A.getZ) * (B.getZ - A.getZ)) / len2
  ...
  def distanceSegmentSegment(A: Coordinate, B: Coordinate, C: Coordinate, D: Coordinate): Double = {
    ...
    val denom = a * c - b * b
    if (java.lang.Double.isNaN(denom)) throw new IllegalArgumentException("Ordinates must not be NaN")
    var s = .0
    var t = .0
    if (denom <= 0.0) {
      /**
