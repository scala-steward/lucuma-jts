...
class OffsetCurveSetBuilder(var inputGeom: Geometry, var distance: Double, var curveBuilder: OffsetCurveBuilder) {
  ...
  private def addLineString(line: LineString): Unit = {
     ...
     */
    if (CoordinateArrays.isRing(coord) && !curveBuilder.getBufferParameters.isSingleSided) addRingBothSides(coord, distance)
    else {
      val curve = curveBuilder.getLineCurve(coord, distance)
      addCurve(curve, Location.EXTERIOR, Location.INTERIOR)
    }
  ...
  private def addPolygon(p: Polygon): Unit = {
     ...
     */
    private def addRingSide(coord: Array[Coordinate], offsetDistance: Double, sideArg: Int, cwLeftLoc: Int, cwRightLoc: Int): Unit = {
