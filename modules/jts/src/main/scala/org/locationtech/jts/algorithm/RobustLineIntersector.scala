...
class RobustLineIntersector() extends LineIntersector {
  ...
  override protected def computeIntersect(p1: Coordinate, p2: Coordinate, q1: Coordinate, q2: Coordinate): Int = {
    ...
    if (Pq1 == 0 || Pq2 == 0 || Qp1 == 0 || Qp2 == 0) {
      ...
      if (p1.equals2D(q1) || p1.equals2D(q2)) intPt(0) = p1
         ...
         */
        if (Pq1 == 0) intPt(0) = new Coordinate(q1)
        else if (Pq2 == 0) intPt(0) = new Coordinate(q2)
  ...
  private def computeCollinearIntersection(p1: Coordinate, p2: Coordinate, q1: Coordinate, q2: Coordinate): Int = {
    ...
    val q1p2q2 = Envelope.intersects(q1, q2, p2)
    if (p1q1p2 && p1q2p2) {
      intPt(0) = q1
      intPt(1) = q2
    ...
    if (p1q1p2 && q1p1q2) {
      intPt(0) = q1
      intPt(1) = p1
      return if (q1 == p1 && !p1q2p2 && !q1p2q2) POINT_INTERSECTION
      else COLLINEAR_INTERSECTION
    }
    if (p1q1p2 && q1p2q2) {
      intPt(0) = q1
      intPt(1) = p2
      return if (q1 == p2 && !p1q2p2 && !q1p1q2) POINT_INTERSECTION
      else COLLINEAR_INTERSECTION
    }
    if (p1q2p2 && q1p1q2) {
      intPt(0) = q2
      intPt(1) = p1
      return if (q2 == p1 && !p1q1p2 && !q1p2q2) POINT_INTERSECTION
      else COLLINEAR_INTERSECTION
    }
    if (p1q2p2 && q1p2q2) {
      intPt(0) = q2
      intPt(1) = p2
      return if (q2 == p2 && !p1q1p2 && !q1p1q2) POINT_INTERSECTION
      else COLLINEAR_INTERSECTION
    }
    NO_INTERSECTION
  }
  ...
  private def intersection(p1: Coordinate, p2: Coordinate, q1: Coordinate, q2: Coordinate) = {
     ...
     */
    if (!isInSegmentEnvelopes(intPt)) { //      System.out.println("Intersection outside segment envelopes: " + intPt);
  ...
  private def isInSegmentEnvelopes(intPt: Coordinate) = {
    ...
    val env1 = new Envelope(inputLines(1)(0), inputLines(1)(1))
